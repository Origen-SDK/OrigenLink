...
class OrigenLinkJtag
  ...
  def do_tlr()
    for step in 1..8 do self.do_cycle(0,1) end
    self.do_cycle(0,0)
  end
  ...
  def do_shift(numbits, value, capturetdo = false, suppresscomments = false, tdocompare = "")
    ...
    for bit in 0..numbits-2 do
      self.do_cycle(value[bit], 0, capturetdo)
    end
    self.do_cycle(value[numbits-1], 1, capturetdo)

    @tdoval = @tdostr.to_i(2) if capturetdo
    
    if not (suppresscomments) and @verbose_enable and capturetdo
      puts "TDO output = 0x" + @tdoval.to_s(16)
    end
    
    if capturetdo and tdocompare != ""
  ...
  def do_ir(numbits, value, options={})
    ...
    if not (options[:suppresscomments]) and @verbose_enable
    ...
    self.do_shift(numbits, value, capturetdo, options[:suppresscomments], options[:tdocompare])
