...
class OrigenLinkSequencer
  ...
  def processmessage(message)
    ...
    case command[0]
    when 'pin_assign'
      self.pin_assign(command[1])
    when 'pin_patternorder'
      self.pin_patternorder(command[1])
    when 'pin_cycle'
      self.pin_cycle(command[1])
    when 'pin_clear'
      self.pin_clear
    when 'pin_format'
      self.pin_format(command[1])
    when 'pin_timing'
      self.pin_timing(command[1])
    else
  ...
  def pin_format(args)
    ...
    new_timeset(tset_key) if not @cycletiming.key?(tset_key)
    @cycletiming[tset_key].delete('rl')
    @cycletiming[tset_key].delete('rh')
    0.step(argarr.length-2, 2) do |index|
      @cycletiming[tset_key][argarr[index+1]] = [] if not @cycletiming[tset_key].key?(argarr[index+1])
  ...
  def process_events(events, pindata)
    ...
    if not events.nil?
      events.each do |pin|
        response[pin] = self.process_pindata(@pinmap[pin], pindata[@patternpinindex[pin]])
      end
    end
    response
  end
